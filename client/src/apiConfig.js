...
// Update the stock
export function updateStock(products) {
   return new Promise((resolve, reject) => {
      // Try with stored access token
      console.log("Updating stock with stored access token...");
      simplePostApi(env.apiBase + "/api/website/updateStock", store.accessToken, { products: products })
         .then(() => {
            console.log("Success with stored access token");
            resolve();
         })
         // Try with acquired access token
         .catch(async () => {
            await getTokenRedirect();
            console.log("Updating stock with acquired access token...");
            simplePostApi(env.apiBase + "/api/website/updateStock", store.accessToken, { products: products })
               .then(() => {
                  console.log("Success with acquired access token");
                  resolve();
               })
               .catch((error) => {
